function doPrt_IEMdepthLocalizers(Subjects)
% makes PRT with only valid trials, sorted by trial number.  A
...
params.nvol = 155;  %ceil(p.expDur/p.TR) - p.nTRsWait;
...
for s=1:length(Subjects)
    ...
    fn=dir([root, sn, '/', sn, '_Behav', '/',sn,'_' expFileName, '_', '*LocalizerRun*.mat']);
    [runs,c]=size(fn);
    ...
    for rr=1:nRuns
        ...
        for sr = 1:length(expSuffix)
            fn2 = sprintf( '%s%s/%s_Behav/%s_%s_sess%02.f_LocalizerRun%02.f.mat' ,...
                root, sn, sn, sn,expFileName, 1, rr);
            disp(sprintf('loading %s...',fn2));
...
%             stim.stimLocs = p.stimLocs(idx,:);
            ...
            % TODO: make the predictors 
            for i = 1:numConds
                idx = find(p.quadrant==i);
                if vOrMS
                    evt_times(i).start = round( (p.delayStart(idx) )./TR)+1;
                    evt_times(i).end = round( (p.delayEnd(idx) )./TR)+1;
                else
                    evt_times(i).start = round( (p.stimStart(idx)) * 1000);
                    evt_times(i).end = round( (p.stimEnd(idx)) * 1000);
                end
            ...
            col = hsv(numConds);
            writePRT(prtName, expFileName, numConds, vOrMS, labels, ...
                evt_times, col,params);
